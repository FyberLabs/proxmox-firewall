...
# Site-specific credentials (generated by create_site_config.sh)
PRIMARY_PROXMOX_API_SECRET="your_proxmox_api_secret"
PRIMARY_NETWORK_PREFIX="10.1"
PRIMARY_DOMAIN="primary.local"
PRIMARY_PROXMOX_HOST="192.168.1.100"

# Device MAC addresses (added by add_device.sh)
PRIMARY_DEVICE_NAME_MAC="xx:xx:xx:xx:xx:xx"
```

**Key Benefits:**
- **Single Source of Truth**: Site config lives in external `.conf` files
- **No Generated Files**: Everything is dynamic, no `.tfvars` file generation
- **Environment Variables**: Terraform receives all input via `TF_VAR_*` environment variables
- **Clean Separation**: Site config, orchestration, and infrastructure are cleanly separated
