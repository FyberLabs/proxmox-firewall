...
- Include network configuration to connect to configuration server
- Needs qemu-guest-agent python3 openssh-server minimum
- ansible
- terraform
```bash
...
```
Save the token secret securely to be used in terraform environment
- Setup ssh server access with known keys for ansible and terraform access
  (Disable or switch to tailscale network only (as all web UI) after full setup)
- Set the hd parameters
- Set it for VM disk backup snapshots, reserve storage for VMs in Proxmox
